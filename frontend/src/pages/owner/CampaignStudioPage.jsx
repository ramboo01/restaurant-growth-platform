...
function CampaignStudioPage() {
  ...
  const handleSendNow = async (id) => {
    if (!window.confirm('Are you sure you want to send this broadcast campaign to the targeted customer segment?')) {
      return;
    }
    try {
      setSendingId(id);
      const res = await sendCampaign(id);
      showToast(`Campaign broadcast sent to ${res.data?.recipientCount || 0} customers!`);
      loadCampaigns();
    } catch (err) {
    ...
    <div className="container-fluid py-4">
            ...
            <div className="d-flex align-items-center">
              ...
              <div>
                ...
                <h3 className="fw-bold mb-0">
                  {campaigns.reduce((sum, c) => sum + Number(c.recipientCount || 0), 0)}
                </h3>
              </div>
            </div>
          </div>
          ...
          <div className="card border-0 shadow-sm rounded-3 p-3 bg-body">
            <div className="d-flex align-items-center">
              ...
              <div>
                ...
                <h3 className="fw-bold mb-0">
                  {campaigns.filter(c => c.discountCode).length}
                </h3>
              </div>
            </div>
          </div>
          ...
          <div className="card border-0 shadow-sm rounded-3 p-3 bg-body">
            <div className="d-flex align-items-center">
              ...
              <div>
                ...
                <h3 className="fw-bold mb-0 text-success">+34%</h3>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="row g-3 text-center">
          <div className="col-6 col-md-3">
            <div className="border rounded p-3 bg-body-tertiary">
              <span className="badge bg-success mb-2">VIP Guests</span>
              <p className="small text-muted mb-0">Highest order frequency & spend</p>
            </div>
          </div>
          <div className="col-6 col-md-3">
            <div className="border rounded p-3 bg-body-tertiary">
              <span className="badge bg-danger mb-2">At Risk (30+ Days)</span>
              <p className="small text-muted mb-0">Inactive guests needing win-back promos</p>
            </div>
          </div>
          <div className="col-6 col-md-3">
            <div className="border rounded p-3 bg-body-tertiary">
              <span className="badge bg-info text-dark mb-2">New Customers</span>
              <p className="small text-muted mb-0">First-time buyers needing 2nd order push</p>
            </div>
          </div>
          <div className="col-6 col-md-3">
            <div className="border rounded p-3 bg-body-tertiary">
              <span className="badge bg-secondary mb-2">All Customers</span>
              <p className="small text-muted mb-0">Entire registered database list</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="card border-0 shadow-sm rounded-3">
        ...
        <div className="card-body p-0">
          {loading ? (
            <div className="p-4"><LoadingState message="Loading campaign studio..." /></div>
          ) : error ? (
                    ...
                    <tr key={c.id}>
                      ...
                      <td>
                        <span className={`badge ${getChannelBadge(c.channel)}`}>
                          <i className={`bi bi-${c.channel === 'SMS' ? 'chat-dots' : c.channel === 'WhatsApp' ? 'whatsapp' : 'envelope'} me-1`}></i>
                          {c.channel}
                        </span>
                      </td>
                      <td>
                  ...
                  <div className="row g-3">
                    ...
                    <div className="col-md-6">
                      ...
                      >
                        ...
                        <option value="VIP Guests">VIP Guests (Top Spend & Frequent)</option>
                        <option value="At Risk (30+ Days Inactive)">At Risk (30+ Days Inactive)</option>
                        <option value="New Customers">New First-Time Customers</option>
                      </select>
                    </div>
